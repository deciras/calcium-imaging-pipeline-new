    ...
    for pattern in STEP_OUTPUT_PATTERNS:
        for path in out_dir.glob(pattern):
            if path.is_dir():
                shutil.rmtree(path)
                removed += 1
                LOGGER.info("Removed old step-03 directory: %s", path)
...
def read_fps_from_metadata(metadata_path: Path | None, default_fps: float) -> float:
    if metadata_path is None or not metadata_path.exists():
        LOGGER.warning("Metadata was not found; using default fps=%s.", default_fps)
...
def run_caiman_motion_correction(
    ...
    LOGGER.info("Configuring CaImAn cluster for %s.", trial.trial_id)
    _, cluster, actual_processes = cm.cluster.setup_cluster(
        backend="multiprocessing",
        n_processes=n_processes,
        ignore_preexisting=False,
    )
    LOGGER.info("CaImAn cluster process count: %s", actual_processes)
...
def process_trial(
    ...
    if dry_run:
        LOGGER.info("[dry-run] Would run motion correction %s -> %s", trial.movie_path, out_dir)
        return "processed"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not stim_root.exists():
        LOGGER.warning("Stimulus root does not exist; sidecar stimulus files will not be copied: %s", stim_root)
    ...
    LOGGER.info("Stimulus root: %s", stim_root_or_none or "(missing)")
    LOGGER.info("Output root: %s", out_root)
    LOGGER.info("Found %d trial(s) with Max_Proj TIFF files.", len(trials))
