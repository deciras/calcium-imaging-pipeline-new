...
def prepare_original_root(data_root: Path, original_root_name: str, action: str, dry_run: bool) -> Path:
    ...
    if dry_run:
        LOGGER.info("[dry-run] Would ensure original root exists: %s", original_root)
    else:
        original_root.mkdir(parents=True, exist_ok=True)
    ...
    for source in candidates:
        ...
        if destination.exists():
            if action == "skip":
                LOGGER.info("Skipping existing original destination: %s", destination)
                continue
            LOGGER.error("Refusing to overwrite existing original item: %s", destination)
            continue

        if dry_run:
            LOGGER.info("[dry-run] Would move original item: %s -> %s", source, destination)
        else:
            shutil.move(str(source), str(destination))
            LOGGER.info("Moved original item: %s -> %s", source, destination)
...
def update_metadata_csv(
    ...
    if not new_ids:
        LOGGER.info("Metadata is already up to date: %s", csv_path)
        return 0
...
def apply_move_plan(
    ...
    for plan in plans:
        if plan.destination.exists() and action == "skip":
            LOGGER.info("Skipping existing destination: %s", plan.destination)
            skipped += 1
            continue
        ...
        try:
            ...
            if plan.destination.exists():
                ...
                if plan.destination.is_dir():
                    LOGGER.error("Refusing to overwrite a directory: %s", plan.destination)
                    failed += 1
                    continue
            ...
            LOGGER.error("Failed to move %s -> %s: %s", plan.source, plan.destination, exc)
            failed += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    LOGGER.info("Original root: %s", original_root)
    LOGGER.info("Layout mode: %s", args.layout)
    LOGGER.info("Action on conflicts: %s", args.action)
    ...
    if not work_folders:
        LOGGER.warning("No folders selected for processing under: %s", data_root)
        return 0
    ...
    LOGGER.info("Summary:")
    LOGGER.info("  folders processed: %d", run_summary.folders_seen)
    ...
    LOGGER.info("  metadata rows added/planned: %d", run_summary.metadata_rows_added)
