...
用途：
- 单独查看 motion corrected movie
- 单独查看 raw movie / spatial high-pass movie

这个脚本只读取现成 TIFF movie 和 metadata，不会重跑 pipeline。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export a single-movie preview video.")
    parser.add_argument("--movie", type=Path, required=True, help="Input TIFF movie path.")
    parser.add_argument("--metadata-json", type=Path, required=True, help="Step-01 metadata.json for acquisition fps.")
    parser.add_argument("--title", required=True, help="Panel title.")
...
def build_figure(title: str, trial_id: str, first_frame: np.ndarray, output_fps: float, effective_speed: float) -> tuple[plt.Figure, dict[str, object]]:
    fig = plt.Figure(figsize=(7.2, 6.8), dpi=120, constrained_layout=True)
    canvas = FigureCanvasAgg(fig)
    gs = fig.add_gridspec(nrows=2, ncols=1, height_ratios=[14, 1.5])
    movie_ax = fig.add_subplot(gs[0, 0])
    progress_ax = fig.add_subplot(gs[1, 0])

    movie_im = movie_ax.imshow(first_frame, cmap="gray", vmin=0.0, vmax=1.0, interpolation="nearest")
    movie_ax.set_title(title, fontsize=16)
    movie_ax.set_xticks([])
    movie_ax.set_yticks([])
...
def render_single_movie_video(
    ...
    stack = load_stack(movie_path)
    if stack.ndim != 3:
        raise ValueError(f"Movie must be a T x H x W array, got: {stack.shape}")
    ...
    low, high = sample_levels(stack, frame_indices)
    first_frame = normalize_frame(stack[frame_indices[0]], low, high, display_step)
    fig, artists = build_figure(title=title, trial_id=trial_id, first_frame=first_frame, output_fps=output_fps, effective_speed=effective_speed)
    ...
    try:
        ...
        for output_idx, frame_idx in enumerate(frame_indices):
            movie_frame = normalize_frame(stack[frame_idx], low, high, display_step)
            artists["movie_im"].set_data(movie_frame)
