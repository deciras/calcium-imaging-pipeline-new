...
# -*- coding: utf-8 -*-
"""
05a_pixelwise_patch_detection.py

目的
----
在 spatial-highpass movie（若存在）、brightness-corrected movie（若存在）或 motion-corrected movie 上，基于刺激前后时间窗做 pixel-wise response map，
同时输出：
...
ONSET_SEC = 1.0                 # 刺激开始后时间窗
OFFSET_SEC = 5.0                # 刺激结束后时间窗
MIN_SUSTAINED_FRAMES = 1        # 最小持续帧数
...
def get_trial_metadata(json_path: Path):
    ...
    try:
        ...
        if physical.get("pixel_size_um") is not None:
            ...
            width_um = physical.get("fov_width_um", physical.get("width", None))
...
def find_trial_inputs(trial_dir: Path):
    # 优先使用 04 输出的 spatial-highpass movie
    ...
    # 若不存在，再回退到旧的 brightness-corrected movie
    ...
    # 若仍不存在，再回退到 motion-corrected movie
    if len(movie_files) == 0:
...
def discover_trials(root_dir: Path):
    ...
    # 三种 movie 都扫描，但最终按 trial_dir 去重；
    # find_trial_inputs 内部会优先选择 spatial_highpass_movie
...
def save_patch_bundle(out_dir: Path,
    ...
    save_float32_tiff(out_dir / "onset_response_map.tif", np.nan_to_num(onset_map, nan=0.0))
    save_float32_tiff(out_dir / "sustained_response_map.tif", np.nan_to_num(sustained_map, nan=0.0))
    save_float32_tiff(out_dir / "offset_response_map.tif", np.nan_to_num(offset_map, nan=0.0))
...
def run_detection_for_maps(onset_map, sustained_map, offset_map,
    ...
    # compatibility, and add ROI-style columns so 05a/05b/04c outputs can be
    # concatenated directly later.
...
def main():
    ...
    print(f"Found {len(trials)} trial(s) with spatial-highpass / brightness / corrected movie inputs")
    if len(trials) == 0:
