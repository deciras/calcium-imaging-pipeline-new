...
目的
----
在 motion-corrected movie 上做全局亮度校正原型，并同时输出三种校正版 movie，
便于人工比较哪种最适合当前数据：

1. subtract_mean   : 每帧减去全图均值，再加参考亮度
2. subtract_median : 每帧减去全图中位数，再加参考亮度
3. divide_mean     : 每帧除以全图均值，再乘参考亮度

同时输出全局亮度轨迹和 before/after 预览图。
...
├── *_brightness_corrected_divide_mean_movie.tif
└── brightness_correction/
    ├── preview_subtract_mean.png
    ├── preview_subtract_median.png
    ├── preview_divide_mean.png
...
OUT_SUFFIXES = {
    ...
    "divide_mean": "_brightness_corrected_divide_mean_movie.tif",
}
...
def save_movie(path: Path, movie: np.ndarray):
    ...
    tf.imwrite(str(path), arr, imagej=SAVE_FLOAT32, bigtiff=True)
...
def get_preview_frame_indices(n_frames: int, fps: float, stim_df: pd.DataFrame | None):
    if stim_df is None or stim_df.empty:
        center = n_frames // 2
        return max(0, center - 1), min(n_frames - 1, center), max(0, center + 1)
...
def process_single_trial(inputs: dict):
    ...
    movies = {
        ...
        "divide_mean": correct_movie_divide(movie, mean_trace, ref_mean),
    }
...
def main():
    ...
    print("=" * 70)
    print("03.5 Brightness correction (3 modes)")
    print("=" * 70)
