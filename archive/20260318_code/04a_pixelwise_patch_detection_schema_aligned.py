...
目的
----
在 brightness-corrected movie（若存在）或 motion-corrected movie 上，基于刺激前后时间窗做 pixel-wise response map，
同时输出：
...
def find_trial_inputs(trial_dir: Path):
    # 优先使用 03.5 输出的 brightness-corrected movie
    ...
    # 若不存在，再回退到 03 输出的 corrected movie
    ...
    if movie_source == "brightness_corrected":
...
def discover_trials(root_dir: Path):
    ...
    # 两种 movie 都扫描，但最终按 trial_dir 去重；
    # find_trial_inputs 内部会优先选择 brightness_corrected_movie
...
def run_detection_for_maps(onset_map, sustained_map, offset_map,
    ...
    # compatibility, and add ROI-style columns so 04a/04b/04c outputs can be
...
def main():
    ...
    print(f"Found {len(trials)} trial(s) with brightness/corrected movie inputs")
    if len(trials) == 0:
