...
def discover_preferred_movies(root_tif_dir):
    ...
    - 优先 *_brightness_corrected_movie.tif
    - 回退 *_corrected_movie.tif
    """
...
def run_suite2p_inplace(
    ...
    if not discovered:
        print(f"[No files] {root_tif_dir} 下没找到 *_brightness_corrected_movie.tif 或 *_corrected_movie.tif")
        return
    ...
    with ProcessPoolExecutor(max_workers=int(n_workers), mp_context=ctx) as ex:
        ...
        for fu in tqdm(as_completed(futures), total=len(futures), desc="Suite2p (inplace)"):
            ...
            for item in discovered:
                ...
                candidate = candidate.replace("_brightness_corrected_movie.tif", "").replace("_corrected_movie.tif", "")
                if candidate == exp_id:
