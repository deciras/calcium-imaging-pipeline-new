...
SUITE2P_DISPLAY_MODE = "iscell_only"
...
def discover_trials(root_dir: Path):
    trial_dirs = set()
    for p in root_dir.rglob("*_spatial_highpass_movie.tif"):
        trial_dirs.add(p.parent)
    for p in root_dir.rglob("*_brightness_corrected_movie.tif"):
        trial_dirs.add(p.parent)
    for p in root_dir.rglob("*_corrected_movie.tif"):
