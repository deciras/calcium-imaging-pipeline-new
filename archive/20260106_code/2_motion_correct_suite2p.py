...
def get_params_from_json(json_path):
    ...
    try:
        ...
        n_planes = 1  # Avg_Proj 当单 plane 时间序列
        n_channels = int(dims.get("channels", 1))
...
# ============================================================
# 5) 子进程任务：处理一个 Avg_Proj
# ============================================================
def _process_one_avgproj(
    avg_proj_path: str,
    root_tif_dir: str,
    ...
    rel_dir = os.path.relpath(os.path.dirname(avg_proj_path), root_tif_dir)
    stack_name = os.path.basename(avg_proj_path)
    exp_id = stack_name.replace("_Avg_Proj.tif", "").replace("_Avg_Proj.tiff", "")
    ...
    try:
        ...
        log(f"avg_proj_path: {avg_proj_path}")
        log(f"rel_dir      : {rel_dir}")
        log(f"exp_id       : {exp_id}")

        json_path = avg_proj_path.replace("_Avg_Proj.tif", "_metadata.json").replace(
            "_Avg_Proj.tiff", "_metadata.json"
        )
        log(f"json_path    : {json_path}")

        ok, info = sanity_check_tiff_is_time_series_2d(avg_proj_path, min_frames=min_frames)
        if not ok:
        ...
        db = {
            "data_path": [os.path.dirname(avg_proj_path)],
...
# 6) 主函数：并行调度
# ============================================================
def run_suite2p_on_avgproj_2d_tiffs(
    root_tif_dir,
    ...
    for r, _, files in os.walk(root_tif_dir):
        for f in files:
            if f.endswith("_Avg_Proj.tif") or f.endswith("_Avg_Proj.tiff"):
                all_avgproj.append(os.path.join(r, f))
    all_avgproj.sort()

    if not all_avgproj:
        print(f"[No files] {root_tif_dir} 下没找到 *_Avg_Proj.tif(f)")
        return
    ...
    with ProcessPoolExecutor(max_workers=int(n_workers), mp_context=ctx) as ex:
        for p in all_avgproj:
            futures.append(
                ex.submit(
                    _process_one_avgproj,
                    p,
                    root_tif_dir,
                    internal_results_root,
                    organized_results_root,
...
if __name__ == "__main__":
    DATA_PATH = "/mnt/50d357b2-473b-4533-8c74-1db99704876a/yifeiding/calcium_imaging/2025_olympus/Processed_TIF_avg_proj_focus"

    run_suite2p_on_avgproj_2d_tiffs(
        DATA_PATH,
        delete_internal=True,
        min_frames=2,
        n_workers=2,
