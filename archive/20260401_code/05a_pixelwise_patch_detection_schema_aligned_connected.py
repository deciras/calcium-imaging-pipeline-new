...
目的
----
在 spatial-highpass movie（若存在）、brightness-corrected movie（若存在）
或 motion-corrected movie 上，基于两窗比较做 pixel-wise response map：

    baseline window  vs  (stimulus window + post-stim window)
...
# 时间窗（秒）
BASELINE_SEC = 5.0              # 刺激前时间窗
POST_STIM_SEC = 5.0             # 刺激结束后继续纳入 response 的时间窗
...
SAVE_PER_STIM = True
PER_STIM_THRESHOLD_STD = 2.0
PER_STIM_MIN_PATCH_AREA = 15
...
def get_trial_metadata(json_path: Path):
    ...
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        meta["fs"] = float(data.get("temporal_calibration", {}).get("fps", 1.0))
        ...
        if width_um is not None and width_px not in (None, 0):
            meta["pixel_size_um"] = float(width_um) / float(width_px)
    except Exception as e:
...
def find_trial_inputs(trial_dir: Path):
    # 优先使用 04_spatial_highpass 输出的 movie
    movie_files = sorted(trial_dir.glob("*_spatial_highpass_movie.tif"), key=lambda p: natural_key(p.name))
    movie_source = "spatial_highpass"

    # 若不存在，再回退到 brightness-corrected movie
    if len(movie_files) == 0:
        movie_files = sorted(trial_dir.glob("*_brightness_corrected_movie.tif"), key=lambda p: natural_key(p.name))
        movie_source = "brightness_corrected"

    # 若仍不存在，再回退到 corrected movie
    if len(movie_files) == 0:
        movie_files = sorted(trial_dir.glob("*_corrected_movie.tif"), key=lambda p: natural_key(p.name))
        movie_files = [p for p in movie_files if "_brightness_" not in p.name]
    ...
    if movie_source == "spatial_highpass":
        prefix = movie_path.name.replace("_spatial_highpass_movie.tif", "")
...
def discover_trials(root_dir: Path):
    ...
    for movie_path in root_dir.rglob("*_spatial_highpass_movie.tif"):
        trial_dirs.add(movie_path.parent)
    for movie_path in root_dir.rglob("*_brightness_corrected_movie.tif"):
        trial_dirs.add(movie_path.parent)
    for movie_path in root_dir.rglob("*_corrected_movie.tif"):
        if "_brightness_" in movie_path.name:
            continue
        trial_dirs.add(movie_path.parent)
...
def compute_single_event_map(movie: np.ndarray, fps: float, start_sec: float, end_sec: float):
    n_frames = movie.shape[0]
    ...
    post_len = max(1, sec_to_frame(POST_STIM_SEC, fps))

    base_start = max(0, start_frame - baseline_len)
    base_end = start_frame
    resp_start = start_frame
    resp_end = min(n_frames, end_frame + post_len)

    if base_end <= base_start or resp_end <= resp_start:
        return None
    ...
    response_img = frame_window_to_mean(movie, resp_start, resp_end)
    ...
    return {
        ...
        "resp_end": resp_end,
        "response_map": compute_response_map(base_img, response_img, base_std_img, RESPONSE_MODE),
    }
...
def build_detection_map(response_map: np.ndarray, use_abs: bool):
    if response_map is None:
        raise ValueError("No valid response map available for detection.")
    detection_map = np.abs(response_map) if use_abs else response_map
    return np.asarray(detection_map, dtype=np.float32)


def detect_patches_from_map(detection_map: np.ndarray,
                            threshold_std: float,
                            min_area: int,
                            max_area: int):
    finite_vals = detection_map[np.isfinite(detection_map)]
    if finite_vals.size == 0:
        shape = detection_map.shape
        return np.zeros(shape, dtype=bool), np.zeros(shape, dtype=np.int32), 0.0

    sigma = robust_std(finite_vals)
    if sigma <= 0:
        sigma = float(np.std(finite_vals))
    if sigma <= 0:
        sigma = 1.0

    threshold = float(threshold_std) * sigma
    binary = detection_map > threshold
...
def summarize_patches(label_map: np.ndarray,
    ...
    for patch_id in range(1, n_patches + 1):
        ...
        row = {
            ...
            "mean_response": float(np.nanmean(response_map[mask])) if response_map is not None else np.nan,
            "max_response": float(np.nanmax(response_map[mask])) if response_map is not None else np.nan,
            "mean_detection_response": float(np.nanmean(detection_map[mask])),
            "max_detection_response": float(np.nanmax(detection_map[mask])),
        }
    ...
    if not df.empty:
        df = df.sort_values(["max_detection_response", "area_px"], ascending=[False, False]).reset_index(drop=True)
    return df
...
def write_empty_outputs(out_dir: Path, shape_hw, mean_img: np.ndarray, title: str):
    ...
    save_float32_tiff(out_dir / "response_map.tif", empty)
...
def save_patch_bundle(out_dir: Path,
    ...
    save_float32_tiff(out_dir / "response_map.tif", np.nan_to_num(response_map, nan=0.0))
...
def run_detection_for_map(response_map,
                          pixel_size_um,
                          threshold_std,
                          min_area,
                          max_area,
                          use_abs,
                          extra_fields: dict):
    detection_map = build_detection_map(response_map, use_abs=use_abs)
    ...
    if patch_summary is None or patch_summary.empty:
        patch_summary = pd.DataFrame(columns=[
            ...
            "bbox_ymin", "bbox_ymax", "mean_response", "max_response",
            "mean_detection_response", "max_detection_response", "area_um2"
        ])

    # benchmark schema normalization: keep old patch-specific columns for backward
    # compatibility, and add ROI-style columns so outputs can be concatenated later.
    if "patch_id" in patch_summary.columns:
        patch_summary["roi_id"] = patch_summary["patch_id"]
    else:
        patch_summary["roi_id"] = np.nan
    patch_summary["method"] = "pixelwise"
    patch_summary["iscell"] = 1
    patch_summary["iscell_prob"] = np.nan
    patch_summary["suite2p_index"] = np.nan
...
def process_single_trial(inputs: dict):
    ...
    print(f"    movie source: {inputs.get('movie_source', 'unknown')}")
    ...
    for stim_idx, row in stim_df.iterrows():
        out = compute_single_event_map(movie, fps, float(row["start_time_sec"]), float(row["end_time_sec"]))
        if out is None:
            continue

        valid_events.append((stim_idx + 1, row, out))
        response_maps.append(out["response_map"])

        if SAVE_PER_STIM:
            stim_dir = per_stim_root / f"stim_{stim_idx + 1:03d}"
            patch_mask, patch_label_map, patch_summary, threshold = run_detection_for_map(
                response_map=out["response_map"],
                pixel_size_um=pixel_size_um,
                ...
                extra_fields={
                    ...
                    "comparison_mode": "baseline_vs_stim_plus_post",
                    "baseline_start_frame": out["base_start"],
                    "baseline_end_frame": out["base_end"],
                    "response_start_frame": out["resp_start"],
                    "response_end_frame": out["resp_end"],
                },
            )
            save_patch_bundle(
                stim_dir,
    ...
    response_map = nanmean_stack(response_maps, (h, w))
...
def main():
    ...
    print(f"Found {len(trials)} trial(s) with spatial-highpass / brightness-corrected / corrected movie inputs")
    if len(trials) == 0:
