...
def _process_one_maxproj(
    ...
    try:
        ...
        # 检查有无*_brightness_trace.csv,如果有也copy过来
        brightness_trace_path = os.path.join(
            os.path.dirname(max_proj_path),
            f"{exp_id}_brightness_trace.csv"
        )
        if os.path.exists(brightness_trace_path):
            shutil.move(brightness_trace_path, final_exp_folder)
            log("[COPY] brightness_trace.csv copied.")

        brighthess_trace_png_path = os.path.join(
            os.path.dirname(max_proj_path),
            f"{exp_id}_stim_trace.png"
        )
        if os.path.exists(brighthess_trace_png_path):
            shutil.move(brighthess_trace_png_path, final_exp_folder)
            log("[COPY] stim_trace.png copied.")

        # 复制日期文件夹级别的 stim_events.csv 和 stim_map.csv
        # 这两个文件在 Processed_TIF_YYYYMMDD 文件夹，不在每个trial文件夹里
        date_folder = os.path.dirname(os.path.dirname(max_proj_path))  # 向上两级到日期文件夹
        
        stim_events_path = os.path.join(date_folder, "stim_events.csv")
        if os.path.exists(stim_events_path):
            # 复制到结果文件夹的上一级（日期级别）
            date_results_folder = os.path.dirname(final_exp_folder)
            os.makedirs(date_results_folder, exist_ok=True)
            stim_events_dest = os.path.join(date_results_folder, "stim_events.csv")
            # 只复制一次（避免重复复制）
            if not os.path.exists(stim_events_dest):
                shutil.move(stim_events_path, stim_events_dest)
                log("[COPY] stim_events.csv copied to date folder.")
        
        stim_map_path = os.path.join(date_folder, "stim_map.csv")
        if os.path.exists(stim_map_path):
            date_results_folder = os.path.dirname(final_exp_folder)
            os.makedirs(date_results_folder, exist_ok=True)
            stim_map_dest = os.path.join(date_results_folder, "stim_map.csv")
            if not os.path.exists(stim_map_dest):
                shutil.move(stim_map_path, stim_map_dest)
                log("[COPY] stim_map.csv copied to date folder.")
...
if __name__ == "__main__":
    ...
    clean_bash_dir = '/mnt/50d357b2-473b-4533-8c74-1db99704876a/yifeiding/calcium_imaging/olympus_code/20260131_code/03_move_tif.sh'

    subprocess.run(["bash", clean_bash_dir, organized_results_root])
